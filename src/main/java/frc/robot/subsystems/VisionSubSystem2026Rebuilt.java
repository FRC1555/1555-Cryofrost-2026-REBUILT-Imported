...
public class VisionSubSystem2026Rebuilt extends SubsystemBase {
    ...
    private double calculateDistanceFromPitchR(double pitchR) {
        // Example formula: distance = (targetHeight - cameraHeight) / tan(cameraAngle +
        // pitch)
    ...
    /**
     * Get an estimated robot pose in 2D (field frame) from the PhotonPoseEstimator if available.
     * Returns an Optional containing the Pose2d when an estimate is present.
     */
    public Optional<Pose2d> getEstimatedPose2d() {
        PhotonPipelineResult resultR = cameraR.getLatestResult();
        return photonEstimatorR.estimateLowestAmbiguityPose(resultR).map(est -> {
    ...
    PhotonPoseEstimator photonEstimatorR = new PhotonPoseEstimator(
            ...
            new Transform3d(
                    new Translation3d(0.0, 0.2, 0.58), // x, y, z offsets in meters
